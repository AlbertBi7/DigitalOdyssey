...
while true; do
    ...
    if [ -d "$CURRENT_DIR/$choice" ]; then
        ...
        if [ -f "$CURRENT_DIR/Coin.txt" ]; then
            echo "You have Reaced a Dead End ...Go back"
	...
	if [ -f "$CURRENT_DIR/Next.txt" ]; then
   		  echo "🚪 You found the path to Ring2!"
        	  echo "🔑 Password for Ring2: $password_ring2"
        fi
